...
type CostInput struct {
	...
	UsageUnits                float64 // 音频/视频等连续计量单位（分钟/小时/百万字符/秒）
	SizeTier                  string  // 按次/图片/视频模式的层级标签（"1K","2K","4K","HD","480p","720p" 等）
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	...
	case BillingModeVideo:
		// 视频模式支持两种计费方式：
		// 1. 按秒计费（UsageUnits 或 DurationSeconds * RequestCount）
		// 2. 按次计费（仅 RequestCount）
		if input.UsageUnits > 0 || input.DurationSeconds > 0 {
